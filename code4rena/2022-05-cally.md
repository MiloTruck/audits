...
| - | - | - |
| M-01 | Prevent users from sending more ETH than the premium price in `buyOption()` | Medium |
| M-02 | Missing upper limit definition in `setFee()` | Medium |
| M-03 | Vaults created with fee-on-transfer tokens will always fail | Medium |
| M-04 | Recommend using `safeTransferFrom()` instead of `transferFrom()` for NFTs | Medium |
