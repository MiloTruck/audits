...
However, in the [LSP-20 specification](https://github.com/lukso-network/LIPs/blob/main/LSPs/LSP-20-CallVerification.md), only `_INTERFACEID_LSP20_CALL_VERIFIER` is mentioned. This might be confusing for developers that wish to implement LSP-20, but are unable to figure out which interface ID to use. 
